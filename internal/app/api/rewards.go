...
func (api *API) rewardTTS(w http.ResponseWriter, r *http.Request) {
	...
	err = api.db.UpsertTwitchReward(r.Context(), user.ID, characterID, resp.Data.ChannelCustomRewards[0].ID, db.TwitchRewardTTS)
	if err != nil {
		_ = html.ExecuteTemplate(w, "error.html", &htmlErr{
			ErrorCode:    http.StatusInternalServerError,
