package twitch

import (
	"app/db"
	"app/pkg/tools"
	"encoding/json"
	"fmt"
	"io"
	"net/http"
	"net/url"
	"strconv"
	"strings"

	"github.com/nicklaw5/helix/v2"
)

type Config struct {
	Secret   string `yaml:"secret"`
	ClientID string `yaml:"client_id"`
}

var _ HTTPClient = http.DefaultClient

type HTTPClient interface {
	Do(*http.Request) (*http.Response, error)
}

type Client struct {
	httpClient HTTPClient
	cfg        *Config
}

func New(httpClient HTTPClient, cfg *Config) *Client {
	return &Client{
		httpClient: httpClient,
		cfg:        cfg,
	}
}

type userTokenData struct {
	AccessToken  string   `json:"access_token"`
	ExpiresIn    int64    `json:"expires_in"`
	RefreshToken string   `json:"refresh_token"`
	Scope        []string `json:"scope"`
}

// CodeHandler exchanges an OAuth code for tokens. host must be the host the
// authorize redirect actually used — Twitch rejects the exchange when
// redirect_uri differs from the one in the authorize request.
func (c *Client) CodeHandler(code string, host string) (*db.User, *db.TwitchTokens, error) {
	data := url.Values{}
	data.Set("client_id", c.cfg.ClientID)
	data.Set("client_secret", c.cfg.Secret)
	data.Set("code", code)
	data.Set("grant_type", "authorization_code")
	data.Set("redirect_uri", "https://"+host+"/twitch_redirect_handler")

	encodedData := data.Encode()

	resp, err := http.Post("https://id.twitch.tv/oauth2/token", "application/x-www-form-urlencoded", strings.NewReader(encodedData))
	if err != nil {
		return nil, nil, fmt.Errorf("failed to send post request to token url: %w", err)
	}
	defer tools.DrainAndClose(resp.Body)

	respData, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to read the token response body: %w", err)
	}

	if resp.StatusCode > 299 {
		return nil, nil, fmt.Errorf("status code (%d) of post request to token url is invalid: %s", resp.StatusCode, string(respData))
	}

	respJson := &userTokenData{}
	if err = json.Unmarshal(respData, &respJson); err != nil {
		return nil, nil, fmt.Errorf("failed to unmarshal json: %w", err)
	}

	client, err := c.NewHelixClient(respJson.AccessToken, respJson.RefreshToken)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create helix client: %w", err)
	}

	usersResp, err := client.GetUsers(nil)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get users: %w", err)
	}

	if usersResp == nil || len(usersResp.Data.Users) == 0 {
		return nil, nil, fmt.Errorf("no users found")
	}

	userID, err := strconv.Atoi(usersResp.Data.Users[0].ID)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to convert userId to int: %w", err)
	}

	user := &db.User{
		TwitchUserID: userID,
		TwitchLogin:  usersResp.Data.Users[0].Login,
	}
	tokens := &db.TwitchTokens{
		AccessToken:  respJson.AccessToken,
		RefreshToken: respJson.RefreshToken,
		Scopes:       respJson.Scope,
	}

	return user, tokens, nil
}

func (c *Client) NewHelixAppClient() (*helix.Client, error) {
	return helix.NewClient(&helix.Options{
		HTTPClient: c.httpClient,

		ClientID:     c.cfg.ClientID,
		ClientSecret: c.cfg.Secret,
	})
}

func (c *Client) NewHelixClient(accessToken, refreshToken string) (*helix.Client, error) {
	return helix.NewClient(&helix.Options{
		HTTPClient: c.httpClient,

		ClientID:        c.cfg.ClientID,
		ClientSecret:    c.cfg.Secret,
		UserAccessToken: accessToken,
		RefreshToken:    refreshToken,
	})
}
