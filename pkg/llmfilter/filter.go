...
Everything else stays untouched, whatever the surrounding context and no matter how rude it sounds: ordinary profanity in any language, edgy or dark humor, criticism of ideas, beliefs, things or behavior (criticizing a religion or an opinion is not hate; only hating its people is), sexual jokes or fetish talk about adults, insults not based on a protected characteristic, negativity not aimed at a protected group, and violence that is clearly game, fiction or hyperbole. In particular:
...
- Mask as LITTLE as possible: the smallest span — usually a single word, such as a slur, the targeted group, or the hostile verb — whose removal defuses the hate, never a whole clause or sentence when one word is enough. But what REMAINS must itself read as non-hateful: when a group is degraded or dehumanized with attributes (insults, or comparisons such as animals, vermin, dirty, subhuman), mask every such attribute, not just the group's name and not just the first.
- Judge the speaker's stance, not individual words; tag only text that actually expresses hate, a slur, or dehumanization toward people. NOT hateful, even when charged words appear: positive or neutral mentions of a group ("I love jews", "I like black people"), benign negativity aimed at things, food, objects, ideas or general frustration ("I hate pepper", "I hate Mondays"), and denials, refusals, condemnations, warnings, questions or hypotheticals ("racism is wrong", "I would never do that", "stop being a bigot"). The one exception is a slur itself: text-to-speech voices it whether it is used, quoted, reported, asked about or condemned ("he called me a faggot", "saying nigger is not ok"), so a slur word is tagged in every framing.
- The n-word is not the only slur and English is not the only language: EVERY slur is in scope — racial, ethnic, national-origin, caste, religious, anti-gay, anti-trans, disability — in Russian, Ukrainian, Spanish, Portuguese, French, German, Italian, Polish, Turkish, Arabic, Hebrew, Hindi, Japanese, Korean, Chinese or any other language, in its own script or transliterated into Latin letters. If a native speaker would hear it as a slur for a kind of person, tag it, even in a friendly or joking sentence.
- Swearing is NOT a slur: vulgarity in any language (Russian mat, Spanish, Portuguese, German, Turkish, Arabic profanity) stays untouched unless it degrades people over a protected characteristic. A word is a slur because of what it MEANS, never because of the hostile words around it: "you fucking книга" (book), "you absolute пельмень" (dumpling), "shut up you Kartoffel" (potato) are clean, while "shut up you хач", "этот faggot опять" and "sup my nіgga" (Cyrillic і) are tagged. Never tag a foreign word merely for looking crude or unfamiliar.
- Deliberate respellings ARE the slur: swapped vowels, doubled or dropped letters, spacing or hyphens, phonetic spelling, mixed alphabets carrying Cyrillic or Greek lookalike letters, or another language's slur written in Latin letters. Tag "neega", "nekker", "nekkar", "nim guh", "knee grow", "nigga", "n1gga", "n i g g a" and any similar sound-alike, in every language. Judge how the text SOUNDS read aloud, not how it is spelled — text-to-speech will voice it on stream, so if speaking it lands as the slur, tag it. Speech also runs words together, so a slur can straddle a word break: a word can pick up the final consonant of the word before it, leaving both words innocent on the page. Read each word joined to the one before it and tag the word that completes the slur — but only when the join is what carries it; ordinary phrases that merely run into similar sounds stay untouched.
- A word that merely resembles a slur is judged by HOW IT IS USED. For its real meaning it stays untouched: "Nigeria", "Niger", "Nigerian", "niggle", "niggling", "denigrate", "snigger", "knickers", "nickel", "necker", "a chink in the armor", "spick and span", "coon hunting" (raccoons), "the dyke broke" (a levee), "my car's tranny" (transmission), "a fag" (a cigarette in British English), "nekker" (the Witcher monster). But chat reaches for exactly these as a bypass, so one aimed AT a person as an epithet — "you fucking knicker", "shut up snigger", "get out of here nigeria" — stands in for the slur and must be tagged. Ask what the word is doing: naming a place or thing, or name-calling a person? Every language has such lookalikes, including ordinary words that sound like a slur in another language.
...
TARGET: shut up you lunch ink, nobody likes you bee ner
Output: shut up you lunch <f>ink</f>, nobody likes you bee <f>ner</f>

TARGET: how much ink does this printer use, and do not be a nerd about it
Output: how much ink does this printer use, and do not be a nerd about it
...
func (f *Filter) Spans(ctx context.Context, text, custom string) ([]textfilter.Span, error) {
	alone := spokenForm(text) + joinHint(ctx, text) + "TARGET:\n" + text
...
func (f *Filter) ReplySpans(ctx context.Context, prompt, reply, custom string) ([]textfilter.Span, error) {
	alone := spokenForm(reply) + joinHint(ctx, reply) + "TARGET:\n" + reply
