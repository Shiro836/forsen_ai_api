...
- **LLM filter corpus:** `pkg/llmfilter/corpus_*_test.go` (~340 cases, themed: recall / precision / context / streamer rules / robustness) runs against the `oai` block by default; `FILTER_PROVIDER=filter_llm` targets what production runs, `oai_candidate` (or `FILTER_CANDIDATE=1`) a candidate; subtests run 4-wide (`FILTER_PARALLEL`). One provider is ~4 min; `-run Phonetic` (`corpus_phonetic_test.go`) is the ~15 s loop for prompt iteration on homophone/TTS-sound cases. Add a case for every filter bug you fix; the harness prints a `MASKSTATS` line for provider comparison. Prompt changes must be checked on both providers and on real chat (bench kit in `~/.cache/claude-qwen38/rpbench`, see `QWEN38_STATE.md`).
