...
- **LLM filter corpus:** `pkg/llmfilter/corpus_*_test.go` (~340 cases, themed: recall / precision / context / streamer rules / robustness) runs against the `oai` block, or `oai_candidate` with `FILTER_CANDIDATE=1`; subtests run 4-wide (`FILTER_PARALLEL`). One provider is ~4 min. Add a case for every filter bug you fix; the harness prints a `MASKSTATS` line for provider comparison. Prompt changes must be checked on both providers and on real chat (bench kit in `~/.cache/claude-qwen38/rpbench`, see `QWEN38_STATE.md`).
